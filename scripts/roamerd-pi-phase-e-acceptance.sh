...
config_path="${ROAMER_CONFIG:-$repo_dir/config/roamerd.yaml}"
...
restored config/secrets, and a passing scripts/roamerd-pi-preflight.sh result.
EOF
    return 64
  fi
}
...
run_capture preflight "$repo_dir/scripts/roamerd-pi-preflight.sh"
...
log "running real-driver roamerd finite commands"
run_roamerd_capture runtime-status status
run_roamerd_capture runtime-health health
run_roamerd_capture body-sense sense --full
run_roamerd_capture vision-watch watch --output "$acceptance_dir/watch.jpg"
run_roamerd_capture hearing-listen listen --timeout "$listen_timeout" --save-audio "$acceptance_dir/listen.wav"
