...
def load_config(path: Path | None = None) -> dict[str, Any]:
    ...
    Args:
        path: Path to YAML config file, or None to use defaults only.
...
def get_driver_name(config: dict[str, Any], capability: str) -> str:
    ...
    Returns:
        Driver name string.
    """
    return config.get("drivers", {}).get(
        capability, DEFAULT_CONFIG["drivers"].get(capability, "")
    )
...
def get_driver_config(config: dict[str, Any], driver_name: str) -> dict[str, Any]:
    ...
    Returns:
        Driver-specific configuration dictionary.
    """
