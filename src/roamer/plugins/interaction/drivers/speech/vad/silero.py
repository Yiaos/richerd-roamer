...
from roamer.plugins.interaction.drivers.speech.vad.base import VADDriver
...
class SileroDriver(VADDriver):
    ...
    def detect(
        ...
        log(
            f"Processing {len(audio)} samples in {chunk_size}-sample chunks "
            f"with {context_size}-sample context..."
        )
